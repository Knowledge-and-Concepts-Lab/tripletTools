...
`run_embeddings_from_list()` is itself a convenience wrapper: internally it converts `triplet_list` to a pre-indexed CSV and calls `run_embeddings()`, the lower-level function underneath. If your data is already on disk in that pre-indexed `head`/`winner`/`loser`/`sampleSet` format -- e.g. from a large dataset or an external pipeline -- calling `run_embeddings()` directly skips loading everything into an R list first, just to have it written back out to a temp file. Most users won't need this; `run_embeddings_from_list()` is the right entry point when your data is already loaded, in the standard `Center`/`Left`/`Right`/`Answer` format `get.combined()` returns.
