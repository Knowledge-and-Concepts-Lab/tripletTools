...
`condor_apptainer.tmpl` uses HTCondor's `container` universe with `container_image = docker://ghcr.io/...`, the current general-purpose mechanism for this as of this writing. This has **not** been validated against a live HTCondor access point — if your access point rejects `universe = container`, the template includes a commented-out fallback using the older `+SingularityImage` classad; check CHTC's current container documentation, or ask CHTC support, for what your access point expects.
