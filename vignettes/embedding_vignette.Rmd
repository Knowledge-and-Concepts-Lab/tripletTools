...
The setup above assumes R, `tripletTools`, and the `triplet-embeddings` conda environment are already installed on whatever execute node HTCondor matches each job to. If that isn't true everywhere in your pool — or you'd rather not depend on it — this repo also ships everything needed to run each job inside a container instead, via Apptainer (which CHTC's execute nodes use to run Docker images directly, no separate native build required):
...
To use it, point `condor:` at the container template instead in your `params_template.yml` copy:
...
```{r condor-apptainer-setup, engine = "bash"}
...
**Disk requests are larger than `condor.tmpl`'s.** Pulling and unpacking the container image (R + conda + PyTorch) adds several GB on top of whatever the job itself transfers, so `condor_apptainer.tmpl` defaults `request_disk` to `8GB` instead of `2GB`. `params_template.yml` deliberately leaves `request_disk` unset under `defaults:`/`final_fit:` so each template's own default applies rather than one template's value silently overriding the other's — set it explicitly only if you need to raise it further.
