...
**Disk requests are larger than `condor.tmpl`’s.** Pulling and unpacking
the container image (R + conda + PyTorch) adds several GB on top of
whatever the job itself transfers, so `condor_apptainer.tmpl` defaults
`request_disk` to `8GB` instead of `2GB`. `params_template.yml`
deliberately leaves `request_disk` unset under `defaults:`/`final_fit:`
so each template’s own default applies rather than one template’s value
silently overriding the other’s — set it explicitly only if you need to
raise it further.
...
guidance: under 1GB per file stays in `/home`; 1–30GB goes to personal
staging): personal staging areas are at
...
**Avoiding a `docker://` pull on every job:** HTCondor/Apptainer caches
the image per execute node, which is fine at the job counts this
