...
class TestRunFitStageJobCount(unittest.TestCase):
    def test_job_count_is_replicates_times_alphas_squared(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(rs, "submit_jobs_with_retry"):
...
class TestSubmitJobsWithRetry(unittest.TestCase):
    ...
    column, not any stage-specific fields. Mirrors
    test_condor_group_diff_workflow.py's TestSubmitJobsWithRetry, which this
    was ported from."""
    ...
    def test_all_succeed_first_try_calls_build_and_submit_once(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rs.submit_jobs_with_retry(stage_dir, jobs, output_col=1,
